        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 max-w-5xl mx-auto">
          {SOLUTIONS.slice(0, 4).map((solution, index) => {
            ...
            return (
              <div
                key={index}
                className="relative grid grid-cols-1 overflow-hidden rounded-xl border border-border bg-card p-4 sm:p-5"
              >
                <div className="flex items-center gap-2 mb-2">
                  <span
                    className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-r from-[var(--primary)] via-[#FF8A20] to-[var(--accent)] text-white leading-7"
                  >
                    <Icon className="h-4 w-4 aspect-square" />
                  </span>
                  <h3 className="sm:text-lg uppercase font-bold text-lg">{solution.title}</h3>
                </div>
                <p className="text-muted-foreground leading-relaxed font-serif mb-3 text-sm sm:text-lg">
                  {solution.description}
                </p>
                <div className="uppercase sm:text-xl font-extrabold text-[var(--primary)] tracking-wide text-base">{solution.metric}</div>
              </div>
            )
          })}
        </div>
